...
func (r *userRepository) GetByResetPasswordToken(ctx context.Context, token string) (*entity.User, error) {
	result := new(entity.User)
	if err := r.db.WithContext(ctx).Where("reset_token = ?", token).First(&result).Error; err != nil {
		return nil, err
	}
	return result, nil
}
...
func (r *userRepository) GetByVerifyEmailToken(ctx context.Context, token string) (*entity.User, error) {
	result := new(entity.User)
	if err := r.db.WithContext(ctx).Where("verify_token = ?", token).First(&result).Error; err != nil {
		return nil, err
	}
	return result, nil
}
